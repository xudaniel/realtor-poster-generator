...
def fetch_authorized_listing(
    ...
    request = Request(endpoint, headers={config.auth_header: auth_value, "Accept": "application/json", "User-Agent": "realtor-poster-mls/1.4.1"})
    try:
...
def handler_for(config: ConnectorConfig):
    class ConnectorHandler(BaseHTTPRequestHandler):
        server_version = "RealtorPosterMLS/1.4.1"
