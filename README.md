...
[English README](README.en.md) · [v1.4.0 开发版中英双语说明](RELEASE_NOTES_v1.4.0.md) · [v1.3.0 发布说明](RELEASE_NOTES_v1.3.0.md) · [中文产品需求文档](PRD.md) · [English PRD](PRD.en.md) · [在线可视化编辑器 / Live visual editor](https://xudaniel.github.io/realtor-poster-generator/) · [更新记录](CHANGELOG.md) · [参与贡献](CONTRIBUTING.md)

当前稳定版本：**1.3.0** · v1.4 第一阶段开发标识：**1.4.0-dev**
...
- v1.4 第一阶段提供 3–8 项可排序房屋数据栏，并在社交尺寸中按优先级显示四项
- v1.4 第一阶段提供可独立替换、排序、等比显示和焦点裁切的三维/二维双户型图
- v1.4 第一阶段提供最多三项圆形、圆角方形或矩形重点卖点图文
- v1.4 第一阶段提供可排序、隐藏或标记不适用的双语租约详情
- v1.4 第一阶段提供带 MIT 许可 Tabler 图标、可排序并可标记待确认的租金包含项目
...
开发分支的浏览器项目结构升级至第 3 版。房屋数据、双户型图、重点卖点、租约详情和租金包含项目都会进入项目文件、差异比较、来源清单和审批包。所有数值与文字均来自用户输入；系统不会翻译、推断或改写日期、金额、尺寸和租赁条件。

户型图与卖点图片仍只保存在当前设备的浏览器本地存储中。每张户型图会保存原始像素尺寸，并在当前输出尺寸可能不够清晰时给出警告；清单会记录嵌入图片的 SHA-256。Tabler 图标随应用本地提供并根据其 MIT 许可证使用。
