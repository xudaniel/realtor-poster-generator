...
v1.4.1 implements [Issue #22](https://github.com/xudaniel/realtor-poster-generator/issues/22). Static GitHub Pages cannot safely hold MLS/provider secrets, so authorized access uses a loopback connector bound to `127.0.0.1`. It reads the secret only from an environment variable; the browser, repository, logs, project exports, and provenance manifests never receive it. The connector calls only an operator-fixed official or contractual HTTPS endpoint. It provides no scraping, global number search, number guessing, permission bypass, or credential sharing.

Run an authorized connector:
...
Open `http://127.0.0.1:8765`, then connect `http://127.0.0.1:8766` under **Authorized MLS import**. The provider endpoint must return the [normalized contract](docs/MLS_CONNECTOR_CONTRACT.md); each real provider needs an authorized adapter. Import requires one exact provider + board + listing-number match and retains original field values, retrieval time, user overrides, and image-rights state. Withdrawn, expired, ambiguous, unauthorized, and unresolved image-rights cases block export. Refresh shows a diff before protected edits or local images can be replaced, and explicit human review remains mandatory. Missing provider content is never invented, translated, summarized, or rewritten. The workflow does not grant legal, regulatory, MLS®, board, brokerage, or copyright approval.
...
realtor_poster/mls_connector.py     Loopback bridge for an authorized provider
web/                                Manual local editor plus optional authorized import
web/core.js                         Project schema, validation, YAML, manifests, and comparison
web/mls.js                          Loopback-only connector client
...
tests/test_web_mls.js               Browser connector-boundary tests
