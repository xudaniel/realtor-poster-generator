...
- `beds` is the main/legal bedroom count. `bedsAdditional` is optional and may be mapped only when the provider explicitly supplies an additional room or den count. An explicit compound `beds` value such as `2 + 1` is split into the two editor controls while the compound source value is retained in both provenance records. / `beds` 是主要或法定卧室数量；`bedsAdditional` 为可选项，只有供应商明确提供额外房间或书房数量时才可映射。供应商明确返回 `2 + 1` 等组合值时，编辑器会拆分到两个控件，并在两项来源记录中保留原始组合值。
